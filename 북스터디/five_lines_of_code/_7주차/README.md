...
- 이 책을 기준으로 리팩터링을 수행하기 위해 사용될만한 방법이라고 생각함 
  - 충분히 학습하지 않고 이대로 따라하다간... 피를 볼 수도 있을 것 같음.. 
- 눈에 띄게 만들어서 추후에 리팩터링을 유도하기
  - 약간 공감은 안되지만 열거형 사용하기 
  - 기본 제공 타입(정수형, 문자열 등등)을 그냥 사용하기
  - 매직 넘버 사용하기 
  - 정확하고 안전한 내용의 주석 넣기
  - 코드에 공백 넣기(메서드 분리 신호)
  - 이름에 접두사/접미사를 붙여 항목을 그룹화 하기
  - 메서드가 만족스럽지 못하게 추출되었다면 하나로 합쳐서 긴 메서드를 만들기
...
## 14.2 기본 철학 탐구 
- 항상 더 작은 단계 찾기 
  - 큰 문제를 작은 조각으로 분해하는 능력은 프로그래밍의 중요한 부분임
    - 커밋 작게 쪼개기, PR 작게 쪼개기, 메서드 작게 쪼개기 등, 작게 쪼갤수록 되돌리기도 쉽고 작업도 쉽다. 
  - 작은 조각들을 함께 묶을 수 있음 
- 기본 구조 찾기
- 협업을 위한 규칙 사용
  - 규칙은 법이 아닌 도구다 
    - 맹목적인 적용은 위험하다
  - 개발 팀에서는 안전하다는 느낌이 가장 중요하다 
    - 규칙을 서로를 평가하는 도구로 사용하지 말라 
- 개인보다 팀을 우선시하기 
  - 소프트웨어 개발은 팀의 노력이다. 긴밀한 협업에 집중하자 
  - 작업의 병렬화는 서로 공유가 잘 안되는 경우가 많아 해로운 점이 더 많을 수 있다.
  - 빨리 가려면 혼자 가고, 멀리 가려면 함께 가라. 
- 완전성보다 단순성 우선하기
  - 모호하고 일반적인 규칙을 만들지 말자
  - 규칙은 생각 없이 적용할 수 있을 정도로 간단해야 한다. 하지만, 지나친 단순화는 문제가 될 수 있다. 
- 객체 또는 고차 함수 사용하기 
  - 팀에서 더 읽기 쉽다고 생각하는 것을 사용하라 