...
- 생성자를 이용하여 순서를 강제화 할 수 있음
  - 순서를 지키지 않는 것이 불가능하게 만드는게 좋음!

```java
/**
 * As-is
 * - 아래와 같이 구성할 경우, User + email을 사용하는 쪽에서는 email 검증 로직을 반드시 사용해줘야 함 
 * - 사용하는 측면에서 순서를 반드시 지켜줘야 한다는 번거로움 + 누락 가능성 + 낮은 유지보수성 
 */
class User {
    private final String email;
...
/**
 * To-be
 * - 아래와 같이 구성할 경우, User + email을 사용하는 쪽에서 신경을 쓰지 않아도 됨. email에 대한 정책은 User가 가짐.
 *   - 더 나은 방법은, Email이라는 value object를 만들어서 그 쪽에 검증 책임을 위임하는 것이 자연스럽기는 하나, 예제를 간단히 하기 위해 User에서 수행.
 * - 사용하는 측면에서 순서를 반드시 지켜줄 필요가 없음 + 검증 로직이 누락될 가능성이 없음 + 높은 유지보수성  
 */
class User {
    private final String email;
...
- 생성자가 할 수 있는 역할은 어디까지일까? 
  - `Transfer` 예제에서, 해당 생성자에서 `deposit`을 선 수행하게 하는 것이 괜찮은 선택일까? 
  - 아니면 값 검증 정도만 생성자에서 수행하게 하는 것이 맞을까? 
  - 값 캐싱은 어떤가..? 


## 요약
- 캡슐화를 시행하기 위해 데이터 노출을 피하자 
  - getter, setter 사용 자제 및 제거 
  - 비공개 필드 노출을 피해야 함 
- 공통 접사를 사용하지 말자 
  - 공통 접두사나 접미사가 있는 메서드나 변수가 있을 경우, 클래스에 함께 있어야한다는 신호다 
  - 데이터를 캡슐화 하자
- 클래스의 생성자를 이용해 순서를 강제화 할 수 있다
  - 순서라는 불변속성을 제거할 수 있다 
