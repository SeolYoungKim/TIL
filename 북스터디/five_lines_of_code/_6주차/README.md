...
- 복잡성
  - 본질적 복잡성: 도메인 관련 복잡성
  - 우발적 복잡성: 도메인에 관계없는 복잡성 → 우리가 제한해야 할 복잡성 → 변형이 발생하는 지점 도입을 연기하자 
- 확장 가능하게 만드는 방법 
  - 클래스로 타입 코드 대체 
  - 전략 패턴 도입 

## 10.7 추가에 의한 변경으로 이전 버전과의 호환성을 확보 
- 업데이트 시 이전 버전에 대한 호환성을 제공해야 함 
- API의 버전 관리는 어떻게 하고 계신지? 
  - [버저닝 전략](https://velog.io/@eastperson/API-%EB%B2%84%EC%A0%80%EB%8B%9D-%EC%A0%84%EB%9E%B5)

## 10.8 기능 토글로 추가에 의한 변경 
- 환경 변수를 통해 테스트할 수 있음 
- 이는 관련 프로세스가 잘 준비되어 있을 때나 사용할법한 방법인 것 같음 
