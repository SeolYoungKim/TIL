...
### 장점
- 외부에서 도메인으로 향하는 방향이 단방향으로 유지됨 -> 도메인이 순수해짐 
...
- `???`는 클린 아키텍처에서 **험블(Humble)** 이라고 부른다 
  - 테스트 하기 어려운 부분을 의미 
  - 험블 예: GUI, DB
- 라이브러리나 프레임워크는 애플리케이션에서 중요한 요소가 아니니 나중에 정하면 된다
  - 본질인 도메인과 테스트하기 어려운 부분인 험블을 분리
  - 집중할 것에 집중 
- 구분해야 하는 이유 
  - DB, 라이브러리를 바꾼다고 해서, 비즈니스 로직이 변경되어서는 안된다.
  - 본질적으로 분리가 되어야하는 것이 맞다. 
- 본질과 험블을 구분할 수 있어야 하고, 도메인을 설계하는 능력을 갖추자. 


### 서비스를 굳이 추상화해야 하는가?
- 서비스 자체가 유스케이스여야 한다. 
- 서비스는 한번 생성으로 영원히 같은 일을 할 수 있는 객체여야 한다 
- 어차피 유스케이스가 변경되면, 서비스 자체도 분리되어야 한다. 
- 굳이 추상화하지 않아도 클린 아키텍처를 얼추 유지한다. (헥사고날은 아니겠지만...)


### 아키텍처는 정답이 없다
- 헥사고날 해석 방식은 미묘하게 다르다 
- 이러니 저러니 해도 트레이드오프!
  - 아키텍처는 종착지가 아니라 여정에 가깝다
  - 고정된 산출물이 아닌, 계속된 탐구 과정에 더 가까움을 이해하자 
  - 그래야 좋은 아키텍처가 만들어진다. 

  
### CQRS(Command Query Responsibility Segregation)
- 명령과 질의의 책임을 분리하는 것
  - 메소드를(더 넓게는 클래스까지) 명령과 질의로 나누자
- 명령(Command)
  - 상태를 바꾸는 메소드 
  - 리턴은 `void` 타입이어야 함 
  - 편의 상 `return`을 해주는 경우가 있는데, 이러지 말자 
- 질의(Query)
  - 상태를 물어보는 메소드
  - 질의는 상태를 변경해서는 안됨 
