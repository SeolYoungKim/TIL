...
- 테스트용 DB를 사용할 수 있음
  - H2와 같은 DB를 사용할 수 있음 
  - Elastic search, MongoDB와 같이 임베디드 DB를 지원하지 않는 경우, TestContainer 등을 활용해야 하여 테스트가 더 무거워질 수 있음 
...
### 테스트 범위 
- Service, Domain만 테스트 하면 된다고 생각한다
  - Controller: 스프링 팀에서 알아서 잘 테스트할 것임 
  - Repository: JPA + Hibernate가 알아서 잘 테스트할 것임
- 테스트를 작성하는 것도 시간이 든다 
  - 여기까지 작성하는 것은 좀 과하다고 생각함 


### 커버리지 
- Controller, Repository를 테스트하지 않으면 커버리지가 너무 낮게 나온다
  - 도메인이 그만큼 빈약하다는 뜻
  - 서비스의 경쟁력을 의심해봐야... 
- CRUD를 제외하면 도메인이 없어요 
  - 조금 크기가 큰 TODO 앱 정도..
  - 그를 위해 꼭 테스트를 넣을 필요가 있는가


### 의존성을 줄이자 
- 테스트 코드를 작성하는게 귀찮다?
  - 불필요한 의존성이 있는건 아닐지 생각 해보자 
  - 사용하지도 않는 기능이 있는 객체에 의존하고 있지는 않은지 생각해보자